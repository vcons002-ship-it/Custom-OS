...
| **Init / supervisor** | Custom Rust `weaved` (PID 1) | systemd replaced: Clade's service graph is six fixed services; a purpose-built ~runit-scope init keeps PID 1 auditable and boots straight into the Weave. |
...
| **Escalation reasoning** | **Provider-swappable** behind `gated`: a frontier cloud API (Claude or Gemini) *or* a self-hosted model server on the owner's GPU (Ollama / llama.cpp serving a Gemma-27B-class model) | The router and Capability manifests are engine-agnostic (MCP-shaped tools). The reference deployment uses the owner's own GPU as the heavy tier — the purest "OS you own" — with a cloud API as opt-in fallback. Model IDs and pricing verified at build time. |
