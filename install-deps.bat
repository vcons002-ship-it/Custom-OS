...
wsl -d %DISTRO% --cd "%~dp0." -- bash -lc "set -e; echo '[install-deps] syncing repo into WSL filesystem...'; mkdir -p $HOME/clade/Custom-OS; cp -a . $HOME/clade/Custom-OS/; cd $HOME/clade/Custom-OS; sed -i 's/\r$//' tools/*.sh kernel/buildroot-external/board/clade/*.sh; exec bash tools/setup.sh"
